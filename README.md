...
- **`branch-code-reviewer`** — same quality/correctness/maintainability criteria as `code-reviewer`, but scoped strictly to the diff between a target branch (default the current branch) and a base branch (default `main`) via `git diff <base>...<target>`, rather than the whole project. Ask it to "review `<branch>` against `<base>`" to target a specific branch/base pair. Ends its response with a trailing JSON block (`path`/`line`/`start_line`/`severity`/`body` per finding) for tooling like `forge-dispatch` to post as inline PR review comments. Use proactively before opening a PR.
...
- **`branch-security-reviewer`** — same vulnerability classes as `security-reviewer`, but scoped strictly to the diff between a target branch (default the current branch) and a base branch (default `main`) via `git diff <base>...<target>`, rather than the whole project. Ask it to "review `<branch>` against `<base>`" to target a specific branch/base pair. Ends its response with a trailing JSON block (`path`/`line`/`start_line`/`severity`/`body` per finding) for tooling like `forge-dispatch` to post as inline PR review comments. Use proactively before opening a PR touching auth, input handling, file I/O, network calls, deserialization, secrets, or dependencies.
